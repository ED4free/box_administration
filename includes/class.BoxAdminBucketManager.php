...
class Box_Admin_Bucket_Manager {
  ...
  static function download_file( $fileName ) {
    ...
    exec (
      'sudo ' . SYNC_BLOG_SCRIPT . " '$bucket_path' '$local_path' 2>&1",
      $_GET[ 'scr_output' ],
      $script_return
    );
    if ( $script_return ) {
      foreach( $_GET[ 'scr_output' ] as $line ) {
        echo( "$line " );
      }
      throw new Exception();
    }
  ...
  static function upload_file( $fileName ) {
    ...
    exec (
      'sudo ' . SYNC_BLOG_SCRIPT . " '$local_path' '$bucket_path' 2>&1",
      $_GET[ 'scr_output' ],
      $script_return
    );
    if ( $script_return ) {
  ...
  static function remove_file( $fileName ) {
    ...
    exec (
      'sudo ' . REMOVE_BLOG_SCRIPT . " '$bucket_path' 2>&1",
      $_GET[ 'scr_output' ],
      $script_return
    );
    if ( $script_return ) {
